...
import { BuildOutput } from './cargo/Build';
...
interface BaseCfg extends DebugConfiguration {
    ...
    readonly sourceFileMap: { [key: string]: string };
...
async function parseRustDebugConfig(
    ...
    const env: { [key: string]: string } = raw.env || {};
...
export default class RustConfigProvider implements DebugConfigurationProvider, Disposable {
    ...
    public async resolveDebugConfiguration(
        ...
        if (!ws) {
            throw new Error(
                'Not supported yet: rust debugger for files without vscode workspace',
            );
        }
        const msg = debugCfg.noDebug ? 'Launching' : 'Launching debugger';
        return Context.root(ws, msg).runWith(
            async (ctx): Promise<WrappedConfig> => {
                const cargo = await this.cargo.get(ctx);
                const rustc = await this.rustc.get(ctx);

                const cfg = await ctx.subTask('Parsing configuration', async ctx =>
                    parseRustDebugConfig(ctx, this.cargoWorkspace, rustc, debugCfg),
                );
                    ...
                    await cargo.buildBinary(
                        ...
                        [...cfg.buildFlags, '-p', crate],
                        {
                            onStdout(d: BuildOutput) {
                                if (d.reason !== 'compiler-artifact') {
                                    return;
                                }
                                if (d.target.kind[0] === 'custom-build') {
                                    return;
                                }
                                if (
                                    d.target.crate_types[0] === 'bin' ||
                                    (d.target.kind[0] === 'lib' && d.profile.test)
                                ) {
