...
export async function askCrate(
  ...
  const curManifestDir: string | undefined = await getCurrentManifestDir(
    ctx,
    cargoWorkspace,
  );

  const items: Promise<CrateQuickPickItem[]> = cargoWorkspace
    .get(ctx)
    .then(ws =>
      ws.packages
...
export async function askBuildFlags(
  ...
  const items = cargoWorkspace.get(ctx).then((ws: CargoWorkspace) => {
    ...
    for (const t of pkg.targets) {
      ...
      if (kind === 'lib') {
        ...
        items.push({
          ...
          get buildFlags(): string[] {
            throw new Error(
              `Cannot build an executable from unknown kind '${kind}'`,
            );
          },
        });
      }
  ...
  const s = await window.showQuickPick<QuickPickItem & Buildable>(items, {
    matchOnDescription: true,
    matchOnDetail: false,
