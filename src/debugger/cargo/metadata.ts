...
export class MetadataFactory extends CachingFactory<Metadata> {
  ...
  constructor(private readonly cargo: Factory<Cargo>) {
    super([cargo]);
  ...
  public async get(ctx: Context): Promise<Metadata> {
  ...
  public async get_uncached(ctx: Context): Promise<Metadata> {
...
export function parsePkgId(
  ...
  const ss = pkgId.split(' ', 3);
  if (ss.length !== 3) {
    throw new Error(
      `Cannot parse '${pkgId}' as a cargo package because it's length is ${
      ss.length
      } when splited by a space`,
    );
  }

  // Remove surrounding paren
  const replaced = ss[2]
    .substr(1, ss[2].length - 2)
    .replace('path+file', 'file')
    .replace('registry+http', 'https');

  try {
    const path = Uri.parse(replaced);
